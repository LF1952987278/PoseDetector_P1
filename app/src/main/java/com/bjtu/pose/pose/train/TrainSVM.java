...
import static com.bjtu.pose.pose.utils.Functions.listDataSetFiles;
...
import com.bjtu.pose.pose.svm.SvmProcess;
...
public class TrainSVM {
    ...
    static class Companion {
        static void trainModel() {
            // All Params
            System.out.println("phone pose = "+ Constants.phonePose +", seq len = "+ Constants.seqLen +", stride = "+ Constants.stride +", reject sec = "+ Constants.rejectSec +".");

            Map<String, List<String>> posePathMap = new HashMap<>();
            ArrayList<String> list1 = new ArrayList<>();
            list1.add(Constants.flatPath);
            posePathMap.put(Constants.FLAT, list1);
            list1.clear();
            list1.add(Constants.callingPath);
            posePathMap.put(Constants.CALLING, list1);
            list1.clear();
            list1.add(Constants.pocketPath);
            posePathMap.put(Constants.POCKET, list1);
            list1.clear();
            list1.add(Constants.bagPath);
            posePathMap.put(Constants.BAG, list1);
            ...
            posePathMap.forEach((key, value)->{
                if (Constants.phonePose != Constants.ALL && key != Constants.phonePose)
                    return;
                for (String _path : value) {
                    Map<String, List<String>> filePathMap = listDataSetFiles(_path, key);//
                    filePathMap.forEach((k, v)->{
                        System.out.println("---------------"+k+"-------------------------");
                        for(String filePath : v) {
                            System.out.println("read "+ filePath);
                            Map<Long, List<SenUnitV2>> tsSenMap = parseSenFromFile(filePath);//
                            RawFtrLists rawFtrItem = jointSenUnit2FtrStr(tsSenMap, Constants.seqLen, Constants.stride, Constants.rejectSec, key);
...
//            Array<DoubleArray> trainDataV1 = dataListConver.toTypedArray();   // This Test OK !!
            double[][] trainDataV1 = dataListConver;
            int[] groundDataV1 = groundTruthList.stream().mapToInt(Integer::valueOf).toArray();
            SVM<double[]> svm = new SvmProcess(trainDataV1, groundDataV1).getSVM();//
            ...
            for (int i=0; i<testDataList.size(); i++) {
                ...
                for (List<List<List<Double>>> item3 : testDataList.get(i)) {
                    ...
                    if (dlcItem.size() == 45) {
                        ...
                        try {
                            throw new Exception("Very Diff Error Occor! The 45 size is ${dlcItem.size}");
                        } catch (Exception e) {
                            throw new RuntimeException(e);
                        }
                    }
...
//              对testTruth(实际类标签) 和 testlabel（预测类标签）
            ...
            System.out.println("All Test Sets Accuracy Score: "+accuracy(allTestTruth.stream().mapToInt(Integer::valueOf).toArray(), allTestSrLabel.stream().mapToInt(Integer::valueOf).toArray()));
            System.out.println("All Re Test Sets Accuracy score: "+accuracy(allTestTruth.stream().mapToInt(Integer::valueOf).toArray(), allTestReLabel.stream().mapToInt(Integer::valueOf).toArray()));

            // Save SVM
            Object2File.writeObjectToFile(svm, Constants.svmSavePath);
